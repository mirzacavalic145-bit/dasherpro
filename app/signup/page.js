    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--gray-light)', padding: '24px' }}>
      <div style={{ background: 'var(--white)', borderRadius: 16, padding: '40px 36px', maxWidth: 420, width: '100%', boxShadow: '0 4px 32px rgba(0,0,0,.08)' }}>
        ...
        <p style={{ fontSize: 14, color: 'var(--gray)', marginBottom: 28, lineHeight: 1.6 }}>
          Start your 3-day free trial. No charge until day 4.
        </p>
        <form onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          >
            {loading ? 'Creating account...' : plan ? 'Create account & start trial →' : 'Create account →'}
          </button>
        </form>
        <p style={{ textAlign: 'center', fontSize: 13, color: 'var(--gray)', marginTop: 20 }}>
          Already have an account?{' '}
          <Link href={`/login${plan ? `?plan=${plan}` : next ? `?next=${next}` : ''}`} style={{ color: 'var(--green)', fontWeight: 500 }}>
            Sign in
          </Link>
        </p>
        <p style={{ textAlign: 'center', fontSize: 12, color: 'var(--gray)', marginTop: 12, lineHeight: 1.5 }}>
          By signing up you agree to our{' '}
          <Link href="/privacy" style={{ color: 'var(--gray)', textDecoration: 'underline' }}>Privacy Policy</Link>.
        </p>
      </div>
    </div>
  )
